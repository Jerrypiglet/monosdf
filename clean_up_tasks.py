...
for folder in folders:
    ...
    for log_path in log_paths:
        ...
        # print(log_name)
        if_remove = False
        if log_name.startswith('tmp') or log_name.endswith('-tmp') or log_name.endswith('-tmp_new') or log_name.split('_')[-1].endswith('tmp') or any(log_name.startswith(_) for _ in task_list_dates):
            if not if_debug:
